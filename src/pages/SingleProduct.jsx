...
export default function SingleProduct() {
  ...
  return (
    <Flex p={[2, 4]} align="flex-start" mt={['20px', '40px']} padding={{ base: '10px 20px', md: '10px 100px' }} wrap="wrap" direction={{ base: 'column', md: 'row' }}>
      <Box width={{ base: '100%', md: '60%' }} display="flex" alignItems="flex-start" gap={4} flexWrap="wrap" flexDirection={{ base: 'column', md: 'row' }}>
        <Box mt={4} ml={{ base: 0, md: 4 }} width="100%">
          <Image
            src={currentImage || 'default-fallback-image.jpg'}
        ...
        <Box display={{ base: 'none', md: 'flex' }}>
          {productDetail.image?.map((imgUrl, index) => (
            <Image
              key={index}
              src={imgUrl || 'default-thumbnail.jpg'}
